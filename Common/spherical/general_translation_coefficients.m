...
function [ coefficients ] = general_translation_coefficients( N, N_prime, ...
                                            kr, d_alpha, d_beta, type )
% this function implements Eq. (E.5) and (E.6), respectively.
...
% initialize
coefficients = zeros( N, 2*N - 1, N_prime, 2*N_prime - 1 );
    ...
    for m = -n : n
        for n_prime = 0 : N_prime - 1
            for m_prime = -n_prime : n_prime
                ...
                coefficients( n + 1, m + N, n_prime + 1, m_prime + N_prime ) = ...
...
for n_prime_prime = abs( n_prime - n ) : 2 : n_prime + n
    ...
    if ( n_prime_prime >= abs( m_prime_prime ) )
        ...
        coefficient = coefficient + sqrt( ( ( 2*n_prime+1 ) * ( 2*n_prime_prime+1 ) * ( 2*n+1 ) ) / (4*pi) ) .* ...
                i^( n + n_prime_prime - n_prime ) .* ... (-1)^( n_prime_prime ) .* ...
