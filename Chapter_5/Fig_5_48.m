...
for x_0 = -7 : dx_0 : 0
    ...
    % Eq. (5.66) and (5.69)
    d = sqrt( 2*pi*d_ref / ( i * omega/c ) ) * 2 * y_0 ./ Delta .* ( ( 1 - M^2 ) ./ Delta + 1/c * i*omega ) .* exp( i .* omega .* ( t - r_0/c - tau_1 ) ) ./ Delta;
    
    % Eq. (5.71), (5.69), and (3.93)
    d = d + sqrt( 2*pi*d_ref / ( i * omega/c ) ) * 2 * y_0 ./ Delta .* ( ( 1 - M^2 ) ./ Delta - 1/c * i*omega ) .* exp( i .* omega .* ( t - r_0/c - tau_2 ) ) ./ Delta;
