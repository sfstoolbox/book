...
for n = 0 : N - 1
    ...
    if ( strcmp( figure_number, '5.37' ) )
        ...
        if ( n == 0 )
            integral = -1;
        elseif ( rem( n, 2 ) == 1 )
            integral = - i^( n - 1 ) * doublefactorial( n ) / ( n * ( n + 1 ) * doublefactorial( n - 1 ) );
        elseif ( rem( n, 2 ) == 0 )
            integral = 0;
            continue;
        end
        ...
        elseif ( n == 1 )
            integral = -1/3;
        elseif ( rem( n, 2 ) == 0 )
            integral = - i^n * doublefactorial( n - 1 ) / ( ( 2*n + 1 ) * doublefactorial( n - 2 ) ) * ...
                                                ( ( n + 1 ) / ( ( n + 2 ) * n ) - 1 / ( n - 1 ) );
        elseif ( rem( n, 2 ) == 1 )
            integral = 0;
            continue;
    ...
    % from Eq. (5.47)
    S = S + 2*i*sqrt(4*pi) .* k .* integral .* ... 
            i^( -n ) .* sphharm( n, 0, beta, alpha ) .* sphbesselj( n, k.*r ) .* sqrt( 2*n + 1 );
end
