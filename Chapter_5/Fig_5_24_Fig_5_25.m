...
for bin = 1 : size( f, 1 )
    ...
    for index_eta = 1 : no_of_simultaneous_vibration_modes
        ...
        for l = 0 : eta
            ...
            % Eq.(5.25), second case
            w_tilde( bin, k_x ~= 0 ) = w_tilde( bin, k_x ~= 0 ) + ...
                 weight .* (-1)^l .* ( exp( i .* k_x( k_x ~= 0 ) .* x_2 ) - exp( i .* k_x( k_x ~= 0 ) .* x_1 ) );

        end
    end
    
    % factor 1/(i k_x) from Eq.(5.25), second case
    w_tilde( bin, k_x ~= 0 ) = w_tilde( bin, k_x ~= 0 ) ./ ( i .* k_x( k_x ~= 0 ) );
...
transfer_function = transfer_function .* [ sqrt( i .* f ), sqrt( i .* f ) ];
