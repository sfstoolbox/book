...
for m = -M : M
    ...
    % from Eq.(2.38)
    S_ring_m =  4*pi .* i.^( -abs(m) ) .* sphharm( abs(m), -m, phi_pw, theta_pw );

    % from Eq.(2.37a)
    G_ring_m = -i .* k .* sphbesselh( abs(m), 2, k.*R ) .* sphharm( abs(m), -m, pi/2, 0 );   
...
for l = 1 : L
    ...
    exponent  = repmat( exp( i.* ( -M_display : M_display ) .* alpha_0(l) ), [ length(f) 1 ] );
...
for m = -M_display : M_display
    ...
    exponent = repmat( exp( -i .* m .* alpha_0 ), [ length(f) 1 ] );
   ...
   disp( [ 'Calculating order ' num2str( m ) ' of ' num2str( M_display ) '.' ] );
    ...
    for n = abs(m) : M_display + 10 % go towards infinity
        ...
        G_ring_m( :, m+M_display+1 ) = G_ring_m( :, m+M_display+1 ) + ...
                    (-i) .* k .* sphbesselh( n, 2, k.*R ) .* sphharm( n, -m, pi/2, 0 ) .* ...
