...
subfigure = 'a';  % for Fig. 4.16(a) 
%subfigure = 'b';  % for Fig. 4.16(b)
%subfigure = 'c';  % for Fig. 4.16(c)
%subfigure = 'd';  % for Fig. 4.16(d)
%subfigure = 'e';  % for Fig. 4.16(e)
%subfigure = 'f';  % for Fig. 4.16(f)
...
for m = -M : M
    ...
    % Eq.(2.38)
    S_breve( m+M+1 ) = 4*pi .* i.^( -abs(m) ) .* sphharm( abs(m), -m, phi_pw, theta_pw );
...
for l = 1 : L
    ...
    for m = -M : M    
        D = D + 1 / ( 2*pi*R ) .* S_breve( m+M+1 ) ./ G_breve( m+M+1 ) .* exp( i * m* alpha_0( l ) );                
    end
