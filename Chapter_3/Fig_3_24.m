...
for m = -M : M
    ...
    D_exact = D_exact + 1./(2*pi*R) .* ( 4 .* pi .* (-i).^abs(m) .* sphharm( abs(m), -m, phi_pw, theta_pw ) ) ./ ...
                    ( (-i) .* k.* sphbesselh( abs(m), 2, k.*R ) .* sphharm( abs(m), -m, pi/2, 0 ) ) .* exp(i .* m .* alpha_0 );

end

% Eq. (3.101) reformulated for circular distributions
D_wfs = sqrt( 8 * pi * k * d_ref * i ) .* ...
           cos( alpha_0 - theta_pw ) .* ...
                exp( -i .* k .* cos( alpha_0 - theta_pw ) );
