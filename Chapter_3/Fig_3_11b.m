...
fprintf( ['Warning: This script takes awfully long to execute because of the\n' ...
...
% get coefficients (E|E)_(n,n')^(|m|,m'), Eq. (3.55); we could also deduce 
...
G_breve_prime = zeros( N_prime, 2*N_prime - 1 );
...
for n_prime = 0 : N_prime - 1     
    ...
    for m_prime = -n_prime : n_prime
        ...
        G_breve_prime( n_prime + 1, m_prime + N_prime ) = 1i^( -n_prime ) .* ...
    ...
    for m = -n : n
        ...
        for n_prime = 0 : N_prime - 1
            for m_prime = -n_prime : n_prime
                ...
                G_breve_int_mm = G_breve_int_mm + G_breve_prime( n_prime + 1, m_prime + N_prime ) * (-1)^( abs(m) + n_prime ) * ...
                        EI_sect( n_prime + 1, m_prime + N_prime, m + N );
                
                G_breve_int_nm = G_breve_int_nm + G_breve_prime( n_prime + 1, m_prime + N_prime ) * (-1)^( n + n_prime ) * ... 
                        EI( n + 1, m + N, n_prime + 1, m_prime + N_prime );
            
                G_breve_ext = G_breve_ext + G_breve_prime( n_prime + 1, m_prime + N_prime ) * (-1)^( n + n_prime ) * ...
                        EE( n + 1, m + N, n_prime + 1, m_prime + N_prime );
