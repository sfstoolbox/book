...
% both Fig. 3.8(a) and (b) will be created 
...
for n = 0 : N-1
    ...
    for m = -n : n
        ...
        % Eq. (2.38)
        S_breve = 4*pi .* i^(-abs(m)) .* sphharm( abs(m), -m, phi_pw, theta_pw );    
        
        % Eq. (2.37a)
        G_breve_int_nm = -i .* k .* sphbesselh( n, 2, k.*R ) .* sphharm( n, -m, pi/2, 0 );

        % Eq. (2.37a)
        G_breve_int_mm = -i .* k .* sphbesselh( abs(m), 2, k.*R ) .* sphharm( abs(m), -m, pi/2, 0 );

        % synthesized interior field, no scatterer, Eq. (3.50)
        S( r<R ) = S( r<R ) + S_breve .* G_breve_int_nm ./ G_breve_int_mm .* ...
                                sphbesselj( n, k.*r( r<R ) ) .* sphharm( n, m, beta, alpha( r<R ) );
        
        % Eq. (2.37b)
        G_breve_ext = -i .* k .* sphbesselj( n, k.*R ) .* sphharm( n, -m, pi/2, 0 );
    
        % synthesized exterior field, no scatterer, Eq. (3.51)
        S( r>R ) = S( r>R ) + S_breve .* G_breve_ext ./ G_breve_int_mm .* hankel( r>R ) .* sphharm( n, m, beta, alpha( r>R ) );
        
        % Eq. (3.112)
        S_breve_scattered = - bessel_prime ./ hankel_prime .* S_breve .* G_breve_int_nm ./ G_breve_int_mm;
